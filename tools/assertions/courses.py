...
def assert_update_course_response(
    ...
    assert_equal(response.course.estimatedTime, request.estimated_time, "estimated_time")
...
def assert_create_course_response(
    ...
    # Проверяем, что вложенные объекты содержат корректные id
    assert_equal(
        response.course.previewFile.id,
        request.preview_file_id,
        "preview_file_id",
    )
    assert_equal(
        response.course.createdByUser.id,
